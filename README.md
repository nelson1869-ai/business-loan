...
- **Local SQLite Database (`v11`)**: Normalized local storage for borrowers, loans, schedules, repayments, guarantors, emergency contacts, notes, documents, borrower communication logs (device-local history), sync queue, conflicts, and metadata.
...
- **Device-Local Communication History**: Borrower SMS and share actions are tracked locally in `borrower_communication_logs` with explicit status indicators (`openedInSms`, `confirmedSent`, `notSent`, `openedShareSheet`).
...
# 2. Flutter Unit & Widget Tests (including LoanCalculator parity and DatabaseService v11 migrations)
flutter test

# 3. Backend Test Suite & Code Quality
