...
from app.schemas.projections import DashboardProjection, FinancialReportProjection, LoanStatementProjection, ReceiptProjection
...
async def payment_receipt(payment_id: str, db: DbSession, current_user: CurrentUser) -> ReceiptProjection:
    del current_user
...
async def loan_statement(loan_id: str, db: DbSession, current_user: CurrentUser) -> LoanStatementProjection:
    del current_user
...
async def dashboard_projection(db: DbSession, current_user: CurrentUser, as_of: date = Query(default_factory=date.today, alias="asOf")) -> DashboardProjection:
    del current_user
    return await projection_service.dashboard(db, as_of)


@router.get("/reports/financial", response_model=FinancialReportProjection)
async def financial_report(db: DbSession, current_user: CurrentUser, date_from: date = Query(alias="dateFrom"), date_to: date = Query(alias="dateTo")) -> FinancialReportProjection:
    del current_user
