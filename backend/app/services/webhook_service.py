...
def _send_http_post(url: str, data_bytes: bytes, headers: dict[str, str]) -> tuple[int, str]:
...
async def dispatch_n8n_event(event_name: str, payload: dict[str, Any]) -> None:
    ...
    if not webhook_url:
        logger.debug("n8n webhook URL not configured. Skipping event dispatch for '%s'.", event_name)
        return
    ...
    try:
        status_code, resp_text = await asyncio.to_thread(_send_http_post, webhook_url, data_bytes, headers)
