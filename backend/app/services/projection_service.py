...
async def get_receipt(db: AsyncSession, payment_id: str) -> ReceiptProjection | None:
    ...
    reason = reversal.note if reversal is not None and reversal.entry_type == "Reversal" else None
    return ReceiptProjection(
        ...
        reversal_status="Reversal" if payment.entry_type == "Reversal" else "Reversed" if payment.reversal else "Original",
        reversal_reason=reason,
    )


async def get_statement(db: AsyncSession, loan_id: str) -> LoanStatementProjection | None:
    ...
    if loan is None:
        return None
    ledger = sorted(loan.payments, key=lambda item: (item.effective_date, item.created_at, item.id))
    ...
    entries = [
        StatementEntry(
            ...
            running_balance=item.allocation.principal_after + item.allocation.interest_after,
        )
        for item in ledger
    ]
    return LoanStatementProjection(
        generated_at=datetime.now(UTC),
        borrower=PartySummary(id=loan.borrower.id, display_name=f"{loan.borrower.first_name} {loan.borrower.last_name}"),
        ...
        interest_charged=sum((item.expected_interest for item in loan.installments), ZERO),
        interest_collected=sum((p.allocation.applied_interest for p in payments), ZERO) - sum((p.allocation.applied_interest for p in reversals), ZERO),
        penalties=ZERO,
        unapplied_credits=sum((p.allocation.unapplied_credit for p in payments), ZERO) - sum((p.allocation.unapplied_credit for p in reversals), ZERO),
        installment_schedule=[InstallmentResponse.model_validate(item) for item in loan.installments],
...
async def dashboard(db: AsyncSession, as_of: date) -> DashboardProjection:
    ...
    borrower_count = await db.scalar(select(func.count()).select_from(Borrower).where(Borrower.status != "Deleted")) or 0
    loans = list(
        (
            await db.execute(
                select(Loan)
    ...
    due_today = sum((max(item.expected_payment - item.paid_amount, ZERO) for item in due_rows if item.status not in {"Paid", "Cancelled"}), ZERO)
    cash = sum((p.amount for p in payments if p.entry_type == "Payment"), ZERO) - sum((p.amount for p in payments if p.entry_type == "Reversal"), ZERO)
    progress = ZERO if due_today + cash == ZERO else _money(cash * Decimal("100") / (due_today + cash))
    overdue_ids = {
        loan.id
        for loan in loans
    ...
    return DashboardProjection(
        ...
        outstanding_balance=sum((loan.outstanding_principal for loan in loans if loan.status in collectible), ZERO),
        active_loans=sum(loan.status == "Active" and loan.id not in overdue_ids for loan in loans),
        ...
        due_today=due_today,
        due_today_count=sum(item.status not in {"Paid", "Cancelled"} for item in due_rows),
        cash_collected_today=cash,
        collection_progress=progress,
    )


async def financial_report(db: AsyncSession, date_from: date, date_to: date) -> FinancialReportProjection:
    ...
    payments = list(
        (
            await db.execute(
                ...
                .join(Payment.loan)
                .join(Loan.borrower)
                .options(selectinload(Payment.allocation), selectinload(Payment.recorded_by))
                .where(
    ...
    sign = lambda p: Decimal("-1") if p.entry_type == "Reversal" else Decimal("1")
    outstanding = sum((loan.outstanding_principal for loan in loans if loan.status in {"Active", "Overdue", "Defaulted"}), ZERO)
    overdue_loans = [
        loan
        for loan in loans
        if loan.status == "Defaulted"
    ...
    for loan in loans:
        ...
        band = "current" if days == 0 else "1-30" if days <= 30 else "31-60" if days <= 60 else "61-90" if days <= 90 else "91+"
        aging[band] += loan.outstanding_principal
    collectors: dict[str, Decimal] = {}
    for payment in payments:
        collectors[payment.recorded_by.username] = collectors.get(payment.recorded_by.username, ZERO) + sign(payment) * payment.amount
    return FinancialReportProjection(
        date_from=date_from,
        date_to=date_to,
        outstanding_portfolio=outstanding,
        collections=sum((sign(p) * p.amount for p in payments), ZERO),
        interest_earned=sum((sign(p) * p.allocation.applied_interest for p in payments), ZERO),
        principal_collected=sum((sign(p) * p.allocation.applied_principal for p in payments), ZERO),
        unapplied_credits=sum((sign(p) * p.allocation.unapplied_credit for p in payments), ZERO),
        overdue_amount=at_risk,
        portfolio_at_risk=ZERO if outstanding == ZERO else _money(at_risk * Decimal("100") / outstanding),
